...
`tensor-plat-compose-mesh` is centered on platform engineering work in Java. The technical target is: Package a Java local lab for compose analysis with negative fixtures, human-readable error snapshots, and documented operating limits.
...
The score is deterministic. The current threshold is 153. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 2.
